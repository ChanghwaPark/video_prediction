...
def model(FLAGS, gpu_config):
    ...
    #         theta * sigmoid_xent(labels=tf.ones_like(fake0_logit), logits=fake0_logit))
    # else:
    #     loss_disc = tf.reduce_mean(real_logit) - tf.reduce_mean(fake_logit) - theta * tf.reduce_mean(fake0_logit)
    #     loss_fake = - tf.reduce_mean(fake_logit) - theta * tf.reduce_mean(fake0_logit)
    ...
    # else:
    #     clip_disc = [p.assign(tf.clip_by_value(p, -0.01, 0.01)) for p in var_disc]
    #     train_disc = tf.train.RMSPropOptimizer(T.lrD).minimize(-loss_disc, var_list=var_disc)
    ...
    #     train_gen = tf.train.AdamOptimizer(T.lrG, 0.5).minimize(loss_gen, var_list=var_gen)
    # else:
    #     train_gen = tf.train.RMSPropOptimizer(T.lrG).minimize(loss_gen, var_list=var_gen)
    ...
    if FLAGS.phase:
        ...
        with tf.variable_scope('lstm/in'):
            # in_cell = tf.contrib.cudnn_rnn.CudnnLSTM(FLAGS.nhl, FLAGS.nz, dropout=0.5)
        ...
        with tf.variable_scope('lstm/out'):
            # out_cell = tf.contrib.cudnn_rnn.CudnnLSTM(FLAGS.nhl, FLAGS.nz, dropout=0.5)
